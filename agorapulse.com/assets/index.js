...
if (window.location.pathname === '/') {
  if (typeof Cookies.get('langPref') == 'undefined') {
    if (shortLang !== 'en') {
      if (shortLang === 'fr') window.location = '/fr/' + window.location.search;
      if (shortLang === 'es') window.location = '/es/' + window.location.search;
      if (shortLang === 'de') window.location = '/de/' + window.location.search;
    }
  } else {
    if (Cookies.get('langPref') === 'lang-fr') window.location = '/fr/' + window.location.search;
    if (Cookies.get('langPref') === 'lang-es') window.location = '/es/' + window.location.search;
    if (Cookies.get('langPref') === 'lang-de') window.location = '/de/' + window.location.search;
  }
}
...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const wistiaTriggers = document.querySelectorAll('.wistia_embed button,.wistia_embed .button, .wistia_embed .hv2-button-play');
  ...
  if (wistiaTriggers) {
    ...
    for (const wistiaTrigger of wistiaTriggers) {
      wistiaTrigger.addEventListener('click', function (event) {
        ...
        let videoId = wistiaTrigger.dataset.wistiaId;
  ...
  const navbar_style = document.querySelector('.navbar');
  ...
  const add_space_nav = () => {
    if (document.querySelector('.leadinModal-theme-top')) {
      const space_top = document.querySelector('.leadinModal-theme-top').clientHeight;
      navbar_style.style.marginTop = space_top + 'px';
      body_style.style.marginTop = space_top + 'px';
      document.querySelector('.leadinModal-close').addEventListener('click', function () {
        navbar_style.style.marginTop = '0';
        body_style.style.marginTop = '0';
      });
    } else {
      navbar_style.style.marginTop = '0';
      body_style.style.marginTop = '0';
    }
  }; // Non transparent once past header
  ...
  for (var i = 0, linksLength = links.length; i < linksLength; i++) {
    if (
      ...
      !links[i].classList.contains('login-link')
    ) {
  ...
  const platformsDropdown = document.querySelector('#platformsDropdown');
  const resourcesDropdown = document.querySelector('#resourcesDropdown');
  const langDropdown = document.querySelector('#langDropdown');

  if (productDropdown) {
    new Dropdown('#productDropdown', true);
  }

  if (platformsDropdown) {
    new Dropdown('#platformsDropdown', true);
  }

  if (resourcesDropdown) {
    new Dropdown('#resourcesDropdown', true);
  }

  if (langDropdown) {
    new Dropdown('#langDropdown', true);
  ...
  _hsq.push(['setContentType', 'site-page']); //cookies notice


  var createCookieNotice = function (name, value, days, domain) {
    var expires = '';
  ...
  var createCookieBannerTop = function (name, value, days, domain) {
    var expires = '';
    ...
    document.cookie = name + '=' + value + expires + '; domain=' + domain + '; path=/';
  }; //Domain local and beta/prod variable


  if (window.location.hostname === 'localhost') {
    var domain_variable = window.location.hostname;
  } else {
    var domain_variable = 'agorapulse.com';
  }

  var deleteLink = document.querySelector('.cookie-notice-visible');
  var btn_cookies_close = document.querySelector('.close-modal-cookies');
  document.querySelector('.active-cookies').addEventListener('click', () => {
    createCookieNotice('cookie_notice_accepted', true, 30, domain_variable);
    deleteLink.classList.remove('show-cookie');
  });
  btn_cookies_close.addEventListener('click', function (event) {
    event.preventDefault();
    createCookieNotice('cookie_notice_accepted', true, 30, domain_variable);
    deleteLink.classList.remove('show-cookie');
  });
  var topBannerCookie = document.querySelector('.top-banner-cookie');
  var buttonCookie = document.querySelector('.top-banner-cookie .button-cookie');
  var closeModalCookie = document.querySelector('.top-banner-cookie .close-modal-cookie');

  if (topBannerCookie && buttonCookie) {
    buttonCookie.addEventListener('click', event => {
      event.preventDefault(); //createCookieBannerTop('cookie_top_banner_accepted', true, 30, domain_variable);

      createCookieBannerTop(topBannerCookie.dataset.cookieName, true, 30, domain_variable);
      topBannerCookie.classList.remove('show-cookie'); //window.location.href = event.currentTarget.href;

      window.open(event.currentTarget.href, '_blank');
    });
  }

  if (topBannerCookie && closeModalCookie) {
    closeModalCookie.addEventListener('click', event => {
      event.preventDefault();
      createCookieBannerTop(topBannerCookie.dataset.cookieName, true, 30, domain_variable);
      topBannerCookie.classList.remove('show-cookie');
    });
  ...
  function getCookie(cName) {
    ...
    return res;
  } // Check if a cookie exists otherwise display the cookie consent message


  function cookieMessage() {
    if (!getCookie('cookie_notice_accepted')) {
      deleteLink.classList.add('class', 'show-cookie');
    }

    if (!getCookie(topBannerCookie.dataset.cookieName) && topBannerCookie) {
      topBannerCookie.classList.add('show-cookie');
    }
  }
...
__webpack_require__(/*! C:\Users\mayal\Desktop\www\website-www\src\assets\scripts\index.js */"./src/assets/scripts/index.js");
module.exports = __webpack_require__(/*! C:\Users\mayal\Desktop\www\website-www\src\assets\styles\index.scss */"./src/assets/styles/index.scss");
